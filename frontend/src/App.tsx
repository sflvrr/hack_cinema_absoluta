...
export default function App() {
  ...
  const handleStartAgent = async () => {
    ...
    try {
      ...
      log(`[ERROR] Ошибка подключения к n8n: ${e}`);
      setIsRunning(false);
    }
  };
        ...
        <div className="sidebar">
          <div className="sidebar-header">
            ...
            <select
              ...
              style={{ background: '#1a1f2e', color: 'white', border: '1px solid #333', borderRadius: 4, fontSize: 11, padding: '2px 4px' }}
            >
          ...
          <div className="ticket-detail">
            <div className="detail-section">
              ...
              {activeTicket?.description && (
                <div style={{ fontSize: 12, color: '#aeb6c2', marginBottom: 10 }}>{activeTicket.description}</div>
              )}
              ...
              {system ? (
                ...
                <div style={{ fontSize: 12, color: '#67707f', marginBottom: 10 }}>Загрузка системной информации…</div>
              )}
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button
                  ...
                  disabled={isRunning}
                  style={{ flex: 1, padding: '8px', background: '#3b82f6', color: 'white', border: 'none', borderRadius: 6, cursor: 'pointer', fontWeight: 'bold' }}
                >
                ...
                {isRunning && (
                  <button
                    onClick={handleAbort}
                    style={{ padding: '8px 12px', background: '#374151', color: 'white', border: 'none', borderRadius: 6, cursor: 'pointer' }}
                  >
                    ⛔ Abort
                  </button>
                )}
          ...
          <div className="terminal-pane">
            <div className="terminal-header">
              ...
              <button
                ...
                style={{ fontSize: 11, background: 'transparent', color: '#8892a4', border: '1px solid #333', borderRadius: 4, cursor: 'pointer', padding: '2px 8px' }}
              >
            ...
            {proposal && (
              <div className="action-panel">
                ...
                <input className="action-input" value={editCommand} onChange={e => setEditCommand(e.target.value)} />
                <div className="action-buttons">
                  <button className="btn-action btn-approve" onClick={handleApprove}>✅ Approve</button>
            ...
            {showActivity && (
              <div className="action-panel">
                <div style={{ fontSize: 12, color: '#8892a4', marginBottom: 8 }}>
                  Завершение тикета. Отчёт (activity) формирует n8n из audit-log; здесь техник ставит финальный статус.
                </div>
                <button
                  onClick={handleMarkDone}
                  style={{ width: '100%', padding: '8px', background: '#16a34a', color: 'white', border: 'none', borderRadius: 6, cursor: 'pointer', fontWeight: 'bold' }}
                >
                  ✅ Mark ticket as DONE
                </button>
              </div>
